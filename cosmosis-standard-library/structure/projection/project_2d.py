...
class MatterPressurePower3D(Power3D):
    section = "matter_pressure_power_spectrum"
    source_specific = False

class MatterMatterPower3D(Power3D):
    section = "matter_matter_power_spectrum"
    source_specific = False
...
class SpectrumType(Enum):
    ...
    class ShearY(Spectrum):
        power_3d_type = MatterPressurePower3D
        kernels = "W y"
        autocorrelation = False
        name = "shear_y_cl"
        prefactor_power = 1

    class CmbkappaY(Spectrum):
        power_3d_type = MatterPressurePower3D
        kernels = "K y"
        autocorrelation = False
        name = "cmbkappa_y_cl"
    ...
    class YY(Spectrum):
        power_3d_type = PressurePressurePower3D
        kernels = "y y"
        autocorrelation = True
        name = "y_y_cl"
        prefactor_power = 0
...
class SpectrumCalculator(object):
    ...
    def load_kernel(self, block, kernel_name, kernel_dict):
        ...
        if kernel_type == "K" or kernel_type == "y":
            nbin = 1
        else:
        ...
        for i in range(nbin):
            if kernel_type=="N":
                ...
                h = block.get_double(names.cosmological_parameters, "h0")
                kernel = get_tSZ_kernel(self.chi_max, self.a_of_chi, h)
            elif kernel_type=="F":
